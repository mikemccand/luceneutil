...
def runIndexing(args, cpuCount, sparseOrNot, sortOrNot):
  ...
  command = "%s -cp %s perf.IndexTaxis %s %d %s %s %s" % (JAVA_CMD, classPath, indexPath, cpuCount, docsSource, sparseOrNot, sortOrNot)
  ...
  with open(logFile, "wb") as fOut:
    ...
    while True:
      ...
      if m is not None:
        lastMatch = m
        if now > lastPrint + 3.0:
          print("  %6.1f sec: %5.1f M docs; %5.1f K docs/sec" % (float(m.group(1)), int(m.group(2)) / 1000000.0, float(m.group(3)) / 1000.0))
          lastPrint = now
    p.wait()
    if p.returncode != 0:
      print(open(logFile, "r", encoding="utf-8").read())
  ...
  with open(fileName, "rb") as f:
    while True:
      ...
      if s == b"":
        break
      numBytes += len(s)
  print("  done warm %.1f GB" % (numBytes / 1024.0 / 1024.0 / 1024.0))
...
def main():
  ...
  classPath = "%s/lucene/core/build/libs/lucene-core-11.0.0-SNAPSHOT.jar:%s/lucene/analysis/common/build/libs/lucene-analysis-common-11.0.0-SNAPSHOT.jar:%s/../../main" % (
    args.luceneMain,
    args.luceneMain,
    luceneUtilPythonPath,
  )

  run(
    "javac -cp %s %s/../../main/perf/SearchTaxis.java %s/../../main/perf/IndexTaxis.java %s/../../main/perf/DiskUsage.java"
    % (classPath, luceneUtilPythonPath, luceneUtilPythonPath, luceneUtilPythonPath)
  )

  for sparse, sort in (("sparse", True), ("sparse", False), ("nonsparse", False)):
    if sort:
    ...
    print("\nCheckIndex %s" % desc)
    t0 = time.time()
    run("%s -cp %s org.apache.lucene.index.CheckIndex %s" % (JAVA_CMD, classPath, indexPath), "%s/checkIndex%s.log" % (logDir, desc))
    sec = time.time() - t0
    ...
    for iter in range(1):
      ...
      run("%s -cp %s perf.SearchTaxis %s %s" % (JAVA_CMD, classPath, indexPath, sparse), "%s/search%s.%s.log" % (logDir, desc, iter))
