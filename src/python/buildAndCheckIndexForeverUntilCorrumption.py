...
while True:
  ...
  cmd = "%s -cp ROOT/lucene/build/core/classes/java:ROOT/lucene/build/codecs/classes/java org.apache.lucene.index.CheckIndex -crossCheckTermVectors %s/index".replace(
    "ROOT", "%s/%s" % (constants.BASE_DIR, LUCENE_TRUNK_ROOT)
  ) % (JAVA_CMD, INDEX_PATH)
  print("  run: %s" % cmd)
  if os.system(cmd):
