...
def add_paragraph_count_column(input_csv, output_csv):
  ...
  with open(input_csv, encoding="utf-8", newline="") as f_in, open(output_csv, "w", encoding="utf-8", newline="") as f_out:
...
def build_index(csv_file, vec_file, dimensions):
  ...
  print("Building index of wiki_ids...")
  start_time_sec = time.time()
...
def shuffle_and_copy(header_bytes_len, csv_file, vec_file, output_csv, output_vec, wiki_id_index, wiki_ids_in_order, dimensions):
  ...
  print("Reading input files sequentially and writing to shuffled positions...")
  overall_start_time_sec = time.time()

  # Write header to output CSV first
  with open(csv_file, encoding="utf-8") as f_in:
    header_line = f_in.readline()
    print(f"{header_line=}")

  copy_using_write_plan(csv_file, output_csv, total_csv_size + header_bytes_len, csv_write_plan, "CSV", header_line.encode("utf-8"))
...
def main():
  ...
  print("Adding paragraph_count column to CSV...")
