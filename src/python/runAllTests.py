...
CLASSPATH = [
  "../lucene/test-framework/lib/junit-4.10.jar",
  ...
  for dir, subDirs, files in os.walk("%s/solr/core/src/test" % ROOT):
    for file in files:
      if file.endswith(".java") and (file.startswith("Test") or file.endswith("Test.java")):
        ...
        testClass = fullFile[strip:-5].replace("/", ".")
        if testClass in (
          "org.apache.solr.cloud.CloudStateUpdateTest",
          "org.apache.solr.search.TestRealTimeGet",
          "org.apache.solr.servlet.CacheHeaderTest",
          "org.apache.solr.request.TestRemoteStreaming",
          "org.apache.solr.handler.dataimport.TestSqlEntityProcessorDelta2",
        ):
          print("WARNING: skipping test %s" % testClass)
          continue
        # print '  %s' % testClass
