...
def extractIndexStats(indexLog):
  ...
  with open(indexLog, encoding="utf-8") as f:
    while True:
      ...
      if m is not None:
        flushTimeSec += float(m.group(1)) / 1000.0
...
def extractSearchStats(searchLog):
  ...
  for key in (("cab_color:g", None), ("cab_color:g", "longitude"), ("cab_color:y", None), ("cab_color:y", "longitude"), ("cab_color:y cab_color:g", None)):
    l = byQuerySort[key]
    l.sort()
    # median result:
  ...
  key1 = (
    "IndexOrDocValuesQuery(indexQuery=green_pickup_latitude:[40.75 TO 40.9], dvQuery=green_pickup_latitude:[4630931869959979008 TO 4630952980583232307]) IndexOrDocValuesQuery(indexQuery=yellow_pickup_latitude:[40.75 TO 40.9], dvQuery=yellow_pickup_latitude:[4630931869959979008 TO 4630952980583232307])",
    None,
  )
...
def extractDiskUsageStats(logFileName):
  with open(logFileName) as f:
    ...
    while True:
      ...
      mb = int(size.strip().replace(",", "")) / 1024.0 / 1024.0
      mbByPart[what] = mb
    ...
    while True:
      ...
      tup = line.split()
      fieldName = tup[0]
      totMB = int(tup[1].replace(",", "")) / 1024.0 / 1024.0
...
def toMSEpoch(dt):
  ...
  return 1000.0 * (dt - epoch).total_seconds()
    ...
    if os.path.exists("/l/logs.nightly/taxis/%s/results.pk" % fileName):
      ...
      if os.path.exists(logResultsFileName):
        (
          sparseIndexStats,
          nonSparseIndexStats,
          sparseSortedIndexStats,
          sparseSearchStats,
          nonSparseSearchStats,
          sparseSortedSearchStats,
          sparseDiskUsageStats,
          nonSparseDiskUsageStats,
          sparseSortedDiskUsageStats,
        ...
        open(logResultsFileName, "wb").write(
          pickle.dumps(
            (
              sparseIndexStats,
              nonSparseIndexStats,
      ...
      indexDPSData.append((m.groups(), nonSparseIndexStats[0] / 1000.0, sparseIndexStats[0] / 1000.0, sparseSortedIndexStats[0] / 1000.0))
      ...
      docsPerMBRAMData.append((m.groups(), nonSparseIndexStats[3] / 1000.0, sparseIndexStats[3] / 1000.0, sparseSortedIndexStats[3] / 1000.0))
      docsPerMBDiskData.append((m.groups(), nonSparseIndexStats[4] / 1000.0, sparseIndexStats[4] / 1000.0, sparseSortedIndexStats[4] / 1000.0))
      try:
      ...
      searcherHeapMBData.append((m.groups(), toMB(nonSparseSearchStats[0][0]), toMB(sparseSearchStats[0][0]), toMB(sparseSortedSearchStats[0][0])))
      x = [m.groups()]
      ...
      searchBQQPSData.append((m.groups(), msecToQPS(getFastest(nonSparseSearchStats, 6)), msecToQPS(getFastest(sparseSearchStats, 6)), msecToQPS(getFastest(sparseSortedSearchStats, 6))))
      searchRangeQPSData.append((m.groups(), msecToQPS(getFastest(nonSparseSearchStats, 7)), msecToQPS(getFastest(sparseSearchStats, 7)), msecToQPS(getFastest(sparseSortedSearchStats, 7))))
...
""")
    ...
    writeOneGraph(
      ...
      indexSizePartsData,
      "index_size_parts",
      "Index size by part (MB)",
      ("Date", "Doc values (dense)", "Doc values (sparse)", "Doc values (sparse-sorted)", "Points (dense)", "Points (sparse)", "Points (sparse-sorted)"),
    )
    writeOneGraph(
      f,
      indexSizePerFieldData,
      "index_size_by_field",
      "Index size by field (MB)",
      (
        "Date",
        "Dropoff latitude (dense)",
        "Dropoff latitude (sparse)",
        "Dropoff latitude (sparse-sorted)",
        "Fare amount (dense)",
        "Fare amount (sparse)",
        "Fare amount (sparse-sorted)",
        "Dropoff datetime (dense)",
        "Dropoff datetime (sparse)",
        "Dropoff datetime (sparse-sorted)",
      ),
    )
    ...
    writeOneGraph(
      ...
      searcherHeapMBPartData,
      "searcher_heap_parts",
      "Searcher heap used by part (MB)",
      (
        "Date",
        "Postings (dense)",
        "Postings (sparse)",
        "Postings (sparse-sorted)",
        "Doc values (dense)",
        "Doc values (sparse)",
        "Doc values (sparse-sorted)",
        "Stored fields (dense)",
        "Stored fields (sparse)",
        "Stored fields (sparse-sorted)",
        "Points (dense)",
        "Points (sparse)",
        "Points (sparse-sorted)",
      ),
    )
    writeOneGraph(
      f,
      searchSortQPSData,
      "search_sort_qps",
      "TermQuery, sort by longitude (QPS)",
