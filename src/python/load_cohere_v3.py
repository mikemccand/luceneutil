...
assert TOTAL_DOC_COUNT == shuffle_wiki_ids.TOTAL_DOC_COUNT
...
def main():
  ...
  if DO_PARTITION:
    ...
    queries_paragraph_count, docs_paragraph_count = partition_documents(csv_shuffled_file, vec_shuffled_file, csv_source_file, vec_source_file, query_wiki_id_count)
  else:
    queries_paragraph_count = 1_778_739
    docs_paragraph_count = total_paragraph_count - queries_paragraph_count
...
def shuffle_entirely(
  ...
  def shuffle_pair(total_paragraph_count, csv_input, vec_input, csv_output, vec_output, file_label):
    ...
    write_plan = [None] * total_paragraph_count
    ...
    with (
      ...
      for input_pos, csv_row in enumerate(csv_reader):
        ...
        # NOTE: does not handle escaping:
        csv_output_lines[output_pos] = escape_row_to_csv_string(csv_row)
...
def xor_vector_hash(prior_hash_bytes, vector_bytes):
  ...
  Args:
    prior_hash_bytes: bytes object with prior accumulated hash, or None to start
    vector_bytes: bytes object containing the vector data to hash and XOR
  ...
  # if no prior hash, return this hash
  if prior_hash_bytes is None:
