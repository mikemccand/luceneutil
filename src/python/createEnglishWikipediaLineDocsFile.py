...
def maybe_create_binary_file(text_file_name):
  ...
  if not os.path.exists(binary_file_name):
    t0 = time.time()
    subprocess.check_call([sys.executable, "src/python/buildBinaryLineDocs.py", text_file_name, binary_file_name])
    t1 = time.time()
...
def main():
  ...
  if True:
    ...
    if not os.path.exists(pre_random_line_file_name):
      t0 = time.time()
      subprocess.check_call([sys.executable, "src/python/combineWikiFiles.py", "-only-three-columns", line_file_name, extract_file_name, pre_random_line_file_name])
      t1 = time.time()
    ...
    if not os.path.exists(medium_line_file_name):
      t0 = time.time()
      subprocess.check_call([sys.executable, "src/python/splitWikiLineFile.py", big_line_file_name, medium_line_file_name, "1024"])
      t1 = time.time()
