...
def captureEnv(logsDir):
  ...
  for fileName in ("responseTimeTests.py", TASKS_FILE, configFile):
    shutil.copy("%s/%s" % (luceneUtilDir, fileName), "%s/%s" % (logsDir, fileName))
    os.chmod("%s/%s" % (logsDir, fileName), 0o444)

  for fileName in ("/sys/kernel/mm/transparent_hugepage/enabled", "/sys/kernel/mm/redhat_transparent_hugepage/enabled"):
...
def runOne(startTime, desc, dirImpl, postingsFormat, targetQPS, pct=None):
  ...
  print()
  print("%s: config=%s, dir=%s, postingsFormat=%s, QPS=%s %s" % (datetime.datetime.now(), desc, dirImpl, postingsFormat, targetQPS, details))
  ...
  cp = ".:$LUCENE_HOME/build/core/classes/java:$LUCENE_HOME/build/memory/classes/java:$LUCENE_HOME/build/codecs/classes/java:$LUCENE_HOME/build/highlighter/classes/java:$LUCENE_HOME/build/test-framework/classes/java:$LUCENE_HOME/build/queryparser/classes/java:$LUCENE_HOME/build/suggest/classes/java:$LUCENE_HOME/build/analysis/common/classes/java:$LUCENE_HOME/build/grouping/classes/java".replace(
    "$LUCENE_HOME", LUCENE_HOME
  )
  ...
  command = "%s -d %s -l %s/hiccups %s > %s 2>&1" % (JHICCUP_PATH, WARMUP_SEC * 1000, logsDir, command, stdLog)
  ...
  try:
    touchCmd = (
      "%s -Xmx1g -cp .:$LUCENE_HOME/build/core/classes/java:$LUCENE_HOME/build/codecs/classes/java:$LUCENE_HOME/build/highlighter/classes/java:$LUCENE_HOME/build/test-framework/classes/java:$LUCENE_HOME/build/queryparser/classes/java:$LUCENE_HOME/build/suggest/classes/java:$LUCENE_HOME/build/analysis/common/classes/java:$LUCENE_HOME/build/grouping/classes/java perf.OpenCloseIndexWriter %s 2>&1".replace(
        "$LUCENE_HOME", LUCENE_HOME
      )
      % (javaCommand, indexPath)
    )
    ...
    if DO_ZV_ROBOT and desc.startswith("Zing"):
      cmd = "%s -Xmx1g -jar %s %s/ZVRobot %s/ZVRobot.prop > %s/ZVRobot.log 2>&1" % (ORACLE_JVM, ZV_ROBOT_JAR, logsDir, os.path.split(ZV_ROBOT_JAR)[0], logsDir)
    ...
    if CLIENT_HOST is not None:
      # Remote client:
      command = "python -u %s %s %s %s %s %d %.1f results.bin" % (REMOTE_CLIENT, TASKS_FILE, SERVER_HOST, SERVER_PORT, targetQPS, TASKS_PER_CAT, RUN_TIME_SEC)
  ...
  if DO_AUTO_QPS:
    ...
    for job in JOBS:
      ...
      if maxQPS[job] < 2 * AUTO_QPS_START:
        raise RuntimeError("max QPS for job %s (= %s) is < 2*AUTO_QPS_START (= %s)" % (desc, maxQPS[job], AUTO_QPS_START))
    ...
    for pctPoint in AUTO_QPS_PERCENT_POINTS:
      ...
      for job in JOBS:
        ...
        targetQPS = AUTO_QPS_START + (pctPoint / 100.0) * (maxQPS[job] - AUTO_QPS_START)
