...
class SendTasks:
  ...
  def gatherResponses(self):
    ...
    while True:
      ...
      self.results.add(taskString.strip(), totalHitCount, taskStartTime - startTime, latencyMS, queueTimeMS)
      ...
      if now - lastPrint > 2.0:
        ...
        self.out.write("p90: " + str(latenciesInMS[int((len(latenciesInMS) - 1) * 0.9)]))
        self.out.write("p99: " + str(latenciesInMS[int((len(latenciesInMS) - 1) * 0.99)]))
        self.out.write("p999: " + str(latenciesInMS[int((len(latenciesInMS) - 1) * 0.999)]))
        self.out.write("p100: " + str(latenciesInMS[len(latenciesInMS) - 1]))

        self.out.write(
          "%6.1f s: %5.1f%%: %5.1f qps in; %5.1f qps out; %6.1f/%6.1f ms [%d, %d]\n"
          % (now - startTime, pctDone, self.taskID / (now - startTime), actualQPSStats.get(), totalTimeStats.get(), queueTimeStats.get(), self.queue.qsize(), len(self.sent))
        )
        # self.out.flush()
        lastPrint = now
