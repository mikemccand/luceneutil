...
def printResults(results, stats, maxDoc):
  print()
  print("Results on %2fM points:" % (maxDoc / 1000000.0))
...
if nightly:
  if "-timeStamp" in sys.argv:
    ...
    if timeStampDateTime < datetime.datetime(year=2016, month=4, day=15):
      haveRussia = False
    haveFullPolygon = os.path.exists("/l/trunk.nightly/lucene/core/src/java/org/apache/lucene/geo/Polygon.java") or os.path.exists(
      "/l/trunk.nightly/lucene/spatial/src/java/org/apache/lucene/util/Polygon.java"
    )
...
cmd = (
  "javac -cp test-framework/build/classes/java/main:sandbox/build/classes/java/main:codecs/build/classes/java/main:core/build/classes/java/main:sandbox/build/classes/java/main:spatial3d/build/classes/java/main %s"
  % sources
)
print(f"RUN: {cmd}")
if os.system(cmd):
...
with open(logFileName, "w") as log:
  ...
  for shape in ("polyRussia", "polyMedium", "poly 10", "nearest 10", "sort", "distance", "box"):
    for approach in ("points", "geo3d", "shapes"):
      ...
      cmd = (
        "java -XX:+UseParallelGC -Xmx10g -cp %s:test-framework/build/classes/java/main:sandbox/build/classes/java/main:codecs/build/classes/java/main:codecs/build/resources/main:core/build/classes/java/main:core/build/classes/java/main21:core/build/resources/main:sandbox/build/classes/java/main:spatial3d/build/classes/java/main perf.IndexAndSearchOpenStreetMaps -%s -%s%s"
        % (utilSrcDir, approach2, shapeCmd, extra)
      )
      print("RUN: %s" % cmd)
