...
if False:
  for precStep in (4, 8, 12, 16):
    ...
    if not os.path.exists(logFileName):
      ...
      run(
        "java -cp /l/util/build:build/core/classes/java:build/analysis/common/classes/java perf.AutoPrefixPerf /lucenedata/numbers/randlongs.10m.txt /lucenedata/numbers/randlongs.queries.txt /l/indices/numbers %d 0 0 > %s 2>&1"
        % (precStep, logFileName)
      )

    indexTimeSec, indexSizeBytes, totalTerms, bestMS = parseLog(logFileName)
    print("    index sec %.2f" % indexTimeSec)
    print("    index MB %.2f" % (indexSizeBytes / 1024 / 1024.0))
  ...
  for mult in 2, 3, 4, 5, None:
    ...
    if not os.path.exists(logFileName):
      if os.path.exists("/l/indices/numbers"):
        shutil.rmtree("/l/indices/numbers")
      run(
        "java -cp /l/util/build:build/core/classes/java:build/analysis/common/classes/java perf.AutoPrefixPerf /lucenedata/numbers/randlongs.10m.txt /lucenedata/numbers/randlongs.queries.txt /l/indices/numbers 0 %d %d > %s 2>&1"
        % (minItemsInPrefix, maxItemsInPrefix, logFileName)
      )
