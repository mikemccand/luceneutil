...
class SkipTower:
  ...
  def write(self, b, inlined, isFixed):
    if VERBOSE:
      print("SkipTower.write skipPos=%s numNext=%d lastDocID=%s" % (b.pos, len(self.nextTowers), self.lastDocID))
...
class SkipWriter:
  ...
  def finish(self, numPostingsBytes, numDocs):
    ...
    # Add EOF tower:
    endTower = SkipTower(numDocs, NO_MORE_DOCS, numPostingsBytes, self.levelLastTowers[0])
  ...
  def visit(self, docCount, lastDocID, pointer):
    if docCount - self.lastSkipDocCount >= self.skipInterval:
      if VERBOSE:
        print("    save skip: docCount=%d lastDocID=%s pointer=%s" % (docCount, lastDocID, pointer))
...
def writeTowers(skipWriter, fixedDocGap, postingsBytes):
  ...
  while tower is not None:
    ...
    if VERBOSE:
      print("  tower @ pointer=%s lastDocID=%s docCount=%s" % (tower.pointer, tower.lastDocID, tower.docCount))
  ...
  while True:
    ...
    if VERBOSE:
      print("  tower @ pointer=%s lastDocID=%s docCount=%s @ wp=%s (tower.wp=%s)" % (tower.pointer, tower.lastDocID, tower.docCount, b.pos, tower.writePointer))

    assert b.pos == totalBytes + tower.writePointer, "wp=%s expected=%s tower.wp=%d" % (b.pos, totalBytes + tower.writePointer, tower.writePointer)
...
class SkipReader:
  ...
  def readTower(self, pos, lastDocID, targetDocID):
    ...
    assert numLevels <= self.maxNumLevels, "numLevels %s vs maxNumLevels %s" % (numLevels, self.maxNumLevels)
    ...
    while level >= 0:
      ...
      if VERBOSE:
        if isFixed:
          print("  nextPos=%s nextLastDocId=%d nextDocCount=%d" % (self.nextTowerPositions[level], nextTowerLastDocID, self.nextTowerDocCounts[level]))
        else:
          print("  nextPos=%s nextLastDocId=%d" % (self.nextTowerPositions[level], nextTowerLastDocID))
...
def main():
  ...
  if inlined:
    ...
    print("  %.1f%% skip (%d skip bytes; %d postings bytes)" % (pct, len(allBytes) - len(postingsBytes), len(postingsBytes)))
  else:
    skipBytes = writeTowers(sw, fixedDocGap, None)
    pct = 100.0 * (len(skipBytes)) / len(postingsBytes)
    print("  %.1f%% skip (%d skip bytes; %d postings bytes)" % (pct, len(skipBytes), len(postingsBytes)))
        ...
        if sr.skip(targetDocID):
          ...
          if inlined or sr.pointer >= reader.pos:
            ...
            if reader.pos >= len(reader.bytes):
              raise RuntimeError("jumped to pos=%d > length=%d" % (reader.pos, len(reader.bytes)))
            codec.afterSeek()
            if VERBOSE:
            ...
            assert lastDocID == docList[docIDX - 1], "lastDocID=%s expectedLastDocID=%s docIDX=%s" % (lastDocID, docList[docIDX - 1], docIDX)

            idx = docIDX
            while docList[idx] != targetDocID:
              idx += 1

            assert fixedDocGap == 0 or idx - docIDX <= fixedDocGap, "fixedDocGap=%d but skipper was %d away from target" % (fixedDocGap, idx - docIDX)
            del idx
