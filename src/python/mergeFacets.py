  ...
  while True:
    ...
    for i in range(len(shards)):
      ...
      if not exhausted:
        ...
        if VERBOSE:
          print("      shard %d: totalHits=%s len(hits)=%s exhausted=%s lowestCount=%d values=%s" % (i, totalHitCount, len(hits), exhausted, lowestCount, shardMissingValues))
          for value, count in hits:
...
class SameDistrModel:
  ...
  def getShardValues(self, shard):
    ...
    for value, freq in self.freqs.items():
      count = int(docCount * self.r.gauss(freq, 0.1))
      if count > 0:
        results[value] = count
    results = results.items()
    results.sort(cmpByCountThenLabel)
