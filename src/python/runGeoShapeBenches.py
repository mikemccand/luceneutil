...
def printCompareResults(results, stats, maxDoc, resultsBase, statsBase):
  ...
  if "-reindex" in sys.argv or "-reindexFast" in sys.argv:
    ...
    print(
      "|%.1fs|%.1fs|%2.f%%|%.1fs|%.1fs|%2.f%%|%.2f|%.2f|%2.f%%|%.2f|%.2f|%2.f%%|"
      % (
        indexTimeSec,
        indexTimeSecBase,
        computeDiff(indexTimeSec, indexTimeSecBase),
        forceMergeTimeSec,
        forceMergeTimeSecBase,
        computeDiff(forceMergeTimeSec, forceMergeTimeSecBase),
        indexSizeGB,
        indexSizeGBBase,
        computeDiff(indexSizeGB, indexSizeGBBase),
        readerHeapMB,
        readerHeapMBBase,
        computeDiff(readerHeapMB, readerHeapMBBase),
      )
    )
  else:
    print("||Index size (GB)||Reader heap (MB)||")
    ...
    print(
      "|%.2f|%.2f|%2.f%%|%.2f|%.2f|%2.f%%|" % (indexSizeGB, indexSizeGBBase, computeDiff(indexSizeGB, indexSizeGBBase), readerHeapMB, readerHeapMBBase, computeDiff(readerHeapMB, readerHeapMBBase))
    )
  ...
  for op in ops:
    for shape in shapes:
      ...
      if tup in results:
        ...
        print(
          "|%s|%s|%.2f|%.2f|%2.f%%|%.2f|%.2f|%2.f%%|%d|%d|%2.f%%|"
          % (shape, op, mhps, mhpsBas, computeDiff(mhps, mhpsBas), qps, qpsBas, computeDiff(qps, qpsBas), totHits, totHitsBas, computeDiff(totHits, totHitsBas))
        )
